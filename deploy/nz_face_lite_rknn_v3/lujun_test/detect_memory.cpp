...
int main(int argc, char** argv) {
    ...
    int loop = 100000000;
    cv::Mat img = cv::imread("./test_memory.jpg");
    
    unsigned char* buffer = (unsigned char*)malloc(3*img.cols*img.rows);

    for (int i=0; i<loop; i++) {
	    cv::Mat cv_img;
        cv_img.create(img.rows, img.cols, CV_8UC3);
        memcpy(cv_img.data, img.data, 3*img.cols*img.rows);	
	
	std::vector<BoxInfo> boxes = face_detection.Detect(cv_img, 0.6, 0.3);
        printf("cv_img:rows:%d, cols:%d, detect %d face number, loop_times:%d\n", cv_img.rows, cv_img.cols, boxes.size(), i);
    }
    free(buffer);
    return 0;
    //std::vector<BoxInfo> boxes = face_detection.Detect(img, 0.6, 0.3);
    //printf("detect %d face number\n", boxes.size());
    
#if 0
    FILE* det_result = fopen("facedetect_result.txt", "w");
