...
def _process_batch(data, size_index):
    ...
    print('bbox_np_b:', bbox_np_b.shape)
    print('gt_boxes_b:', gt_boxes_b.shape)
    ious = bbox_ious(
...
class Darknet19(nn.Module):
    ...
    def forward(self, im_data, gt_boxes=None, gt_classes=None, dontcare=None,
        ...
        print('conv1s:', conv1s.shape)
        conv2 = self.conv2(conv1s)
        conv3 = self.conv3(conv2)
        print('conv3:', conv3.shape)
        conv1s_reorg = self.reorg(conv1s)
        print('conv1s_reorg:', conv1s_reorg.shape)
        cat_1_3 = torch.cat([conv1s_reorg, conv3], 1)
        print('cat_1_3:', cat_1_3.shape)
        conv4 = self.conv4(cat_1_3)
        print('conv4:', conv4.shape)
        conv5 = self.conv5(conv4)   # batch_size, out_channels, h, w
        print('conv5:', conv5.shape)
        global_average_pool = self.global_average_pool(conv5)
        print('global_average_pool:', global_average_pool.shape)
        ...
        print('global_average_pool_reshaped:', global_average_pool_reshaped.shape)
...
if __name__ == '__main__':
    ...
    print('bbox_pred:', out[0].shape)
    print('iou_pred:', out[1].shape)
    print('prob_pred:', out[2].shape)
