...
#from test_util import test_eval
...
file_root = '/data/xiaojun/data/voc/VOCdevkit/VOC2007/JPEGImages/'
learning_rate = 0.001
num_epochs = 50
batch_size = 32
use_resnet = True
...
for epoch in range(num_epochs):
    ...
    for i,(images,target) in enumerate(train_loader):
        ...
        if (i+1) % 10 == 0:
            ...
            writer.add_scalar("train_loss", loss.detach().cpu().item(), epoch*batch_num_train + i)

    #validation
    validation_loss = 0.0
    net.eval()
    batch_num_val = len(test_loader)
    for i,(images,target) in enumerate(test_loader):
        ...
        if (i+1) % 10 == 0:
            ...
            num_iter += 1
            #vis.plot_train_val(loss_train=total_loss/(i+1))
            writer.add_scalar("test_loss", loss.detach().cpu().item(), epoch*batch_num_val + i)
    validation_loss /= len(test_loader)
    #vis.plot_train_val(loss_val=validation_loss)
    writer.add_scalars("train_val_loss", {"train_loss": total_loss, "val_loss": validation_loss}, epoch)
    ...
    if ap > best_ap:
        best_ap = ap
        print('get best test ap %.5f' % best_ap)
