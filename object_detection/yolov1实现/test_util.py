...
def preds_comp(image_path):
        result = predict_gpu(model,image_path,root_path='/data/xiaojun/data/voc/VOCdevkit/VOC2007/JPEGImages/')
        print("result:", result)
...
def test_eval(model):
    ...
    #    result = predict_gpu(model,image_path,root_path='/data/xiaojun/data/voc/VOCdevkit/VOC2007/JPEGImages/') #result[[left_up,right_bottom,class_name,image_path],]
        #print("result:", result[:10])
    #    for (x1,y1),(x2,y2),class_name,image_id,prob in result: #image_id is actually image_path
    #        preds[class_name].append([image_id,prob,x1,y1,x2,y2])
    pool = mp.Pool(processes = 10)
    #for image_path in image_list:
    #    pool.map(preds_comp, (image_path, ))
    pool.map(preds_comp, image_list)
    ...
    print("preds:", preds)
    pdb
    ap = voc_eval(preds,target,VOC_CLASSES=VOC_CLASSES)
...
if __name__ == "__main__":
    ...
    """
    ck = torch.load('./weights/last.pth')

    #for name, param in ck.items():
